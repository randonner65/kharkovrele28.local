...
//Проверка наличия возвратных положение для схемы на 3 положения
	if($this->exsist($constr, "V") && $schemeX->qpos == 3){//если есть самовозврат и 3 положения
		...
		elseif ($this->exsist($nameswitch, "V2-V0")){
			$NameSwitch["momentfrom"]  = array("value" => "V2-V0", "color" => "black");//запись положение самовозврата в массив имени переключателя
			$PropSwitch["momentfrom"] = "V2-V0";
			$nameswitch = str_replace("V2-V0", "", $nameswitch);//удаление V2-V0
		}
	...
	</br>-из левого (V1-V0);</br>-из правого (V2-V0);</br>-из обоих (V1-V0-V2)";
		}
	}
	...
	if($this->exsist($constr, "P")){
		...
		//print_r($MATCHES);
			...
			$nameswitch = str_replace($MATCHES[0], "", $nameswitch);//удаление наиаенования коробки
			}
...
$uncorrectsecondletter = "";
	...
	if(empty($PropSwitch["box"]) && $PropSwitch["current"] < 30 && $scheme->qcont < 5) $PropSwitch["box"] = "EC400C4";
	else if(empty($PropSwitch["box"]) && $PropSwitch["current"] < 30 && $scheme->qcont < 7) $PropSwitch["box"] = "EC400C5";
	else if(empty($PropSwitch["box"]) && $PropSwitch["current"] < 30 && $scheme->qcont < 11) $PropSwitch["box"] = "EC400C7";
	else if(empty($PropSwitch["box"]) && $PropSwitch["current"] < 70 && $scheme->qcont < 5) $PropSwitch["box"] = "EC400C5";
	else if(empty($PropSwitch["box"]) && $PropSwitch["current"] < 70 && $scheme->qcont < 7) $PropSwitch["box"] = "EC400C6";
	...
	if(($PropSwitch["box"][6] == 4 && $PropSwitch["current"] < 30 && $scheme->qcont > 4) || 
		($PropSwitch["box"][6] == 4 && $PropSwitch["current"] > 30 ) || 
		($PropSwitch["box"][6] == 5 && $PropSwitch["current"] < 30 && $scheme->qcont > 6) || 
		($PropSwitch["box"][6] == 5 && $PropSwitch["current"] > 30 && $PropSwitch["current"] < 70 && $scheme->qcont > 4) || 
		($PropSwitch["box"][6] == 5 && $PropSwitch["current"] > 70) || 
		($PropSwitch["box"][6] == 6 && $PropSwitch["current"] < 30 && $scheme->qcont > 6) ||
		($PropSwitch["box"][6] == 6 && $PropSwitch["current"] > 30 && $scheme->qcont > 4) ||
		($PropSwitch["box"][6] == 7 && $PropSwitch["current"] < 30 && $scheme->qcont > 10) ||
		($PropSwitch["box"][6] == 7 && $PropSwitch["current"] > 30 && $scheme->qcont > 6)){
...
//------------------------------------------------------------------------------------------------------------------------
	...
	else if($this->exsist($PropSwitch["color"],"T")) {
		$PropSwitch["handle"] = "t";
		$PropSwitch["handleText"] = "ручку управления специальной формы типа рукоятка";
		}
	else if($this->exsist($PropSwitch["color"],"R")) {
		$PropSwitch["handle"] = "red";
		$PropSwitch["handleText"] = "красную ручку управления";
		}
	else   {$PropSwitch["handle"] = "black";
...
//Запись способа крепления
	...
	$PropSwitch["fix"] = "P";
	$PropSwitch["fixText"] = "установлен в пластсмассовой коробке с IP54";
	}
...
//Запись самовозврата
	...
	$PropSwitch["moment"] = 1;
	$PropSwitch["momentText"] = "Положения ручки переключателя - с самовозвратом (после отпускания руки она возвращается в исходное положение).";
	}
	else  {
	$PropSwitch["moment"] = 0;
